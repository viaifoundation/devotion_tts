# gen_verse_devotion_indextts2.py
# Index-TTS2 - Zero-shot voice cloning with duration/emotion control
...
# Setup path to find Index-TTS2
INDEXTTS_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../index-tts"))

if os.path.exists(INDEXTTS_PATH):
    sys.path.insert(0, INDEXTTS_PATH)
else:
    print(f"⚠️ Warning: Index-TTS2 path not found at {INDEXTTS_PATH}")
    sys.exit(1)
...
try:
    ...
    print(f"❌ Failed to import Index-TTS2: {e}")
    print(f"Ensure you have cloned the repo to {INDEXTTS_PATH} and installed its requirements.")
    sys.exit(1)
...
CKPT_DIR = os.path.join(INDEXTTS_PATH, "checkpoints")
...
if "-?" in sys.argv or "-h" in sys.argv or "--help" in sys.argv:
    ...
    print("  --emotion TEXT       Emotion prompt (e.g., 'happy', 'sad', 'angry')")
    ...
    print("  python gen_verse_devotion_indextts2.py --input sample.txt")
    print("  # With emotion:")
    print("  python gen_verse_devotion_indextts2.py --input sample.txt --emotion happy")
    sys.exit(0)

# CLI Args
parser = argparse.ArgumentParser(description="Generate Verse Audio with Index-TTS2 (zero-shot voice cloning)")
...
parser.add_argument("--emotion", type=str, default=None, help="Emotion prompt (e.g., 'happy', 'sad', 'angry')")
...
# --- Model Loading ---
print("Loading Index-TTS2 (1.5B) - Duration & Emotion Controllable...")
try:
    use_cuda = torch.cuda.is_available()
    print(f"Loading Index-TTS2... [CUDA={use_cuda}]")
    
    # Initialize Index-TTS2
    tts = IndexTTS(model_dir=CKPT_DIR, device="cuda" if use_cuda else "cpu")
    sample_rate = 24000  # Index-TTS2 default
    
    print(f"✅ Index-TTS2 loaded successfully (sample_rate={sample_rate})")
except Exception as e:
    print(f"❌ Error loading Index-TTS2: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
if verse_ref:
    filename = filename_parser.generate_filename(verse_ref, date_str, extracted_prefix).replace(".mp3", "_indextts2.mp3")
else:
    if extracted_prefix:
        filename = f"{extracted_prefix}_Devotion_{date_str}_indextts2.mp3"
    else:
        filename = f"Devotion_{date_str}_indextts2.mp3"
...
# Note: Index-TTS2 does not currently support speed adjustment
...
def speak(text: str, ref_audio: str = None, ref_text: str = None, emotion: str = None) -> AudioSegment:
    """Convert text to audio using Index-TTS2 zero-shot voice cloning."""
    if DEBUG_LEVEL >= 2:
    ...
    try:
        # Index-TTS2 inference
        if ref_audio and os.path.exists(ref_audio):
            # Use voice cloning with optional emotion
            audio_np = tts.infer(text, prompt=ref_audio, emotion=emotion)
        else:
            audio_np = tts.infer(text, emotion=emotion)
...
# --- Process paragraphs ---
if DEBUG_LEVEL >= 1:
    print(f"Processing {len(paragraphs)} paragraphs with Index-TTS2...")
...
for i, para in enumerate(paragraphs):
    ...
    try:
        segment = speak(para, current_ref_audio, current_ref_text, args.emotion)
        final += segment
